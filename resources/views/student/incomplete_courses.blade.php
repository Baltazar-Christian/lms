...
@section('content')
    <div class="container mt-5">
        <h5 class="mb-4"> <i class="fa fa-book text-warning"></i> Incomplete Courses </h5>
        ...
        <div class="row">
            ...
            <div class="col-md-9">
                ...
                <!-- Display All Courses -->
                <div class="row">
                    @forelse($enrolledCourses as $course1)
                        <div class="col-md-4 col-6 mb-4">
                            <div class="card">

                                <img src="{{ asset('public/storage/covers/' . $course1->course->cover_image) }}" width="150px"
                                    height="150px" class="card-img-top" alt="{{ $course1->course->name }}">
                                <div class="card-body">
                                    ...
                                    <h6 class="card-title text-dark">{{ $course1->course->title }}</h5>
                                        ...
                                        <!-- Enroll button -->
                                        @if (Auth::user()->courses->contains('id', $course1->course->id))
                                            <div class="row">
                                                <div class="col-6">
                                                    @php
                                                        $enrollment=App\Models\Enrollment::where('user_id',Auth::user()->id)->where('course_id',$course1->course->id)->where('approval_status','approved')->latest()->first();
                                                    @endphp
                                                    @if ($enrollment)
                                                        <a href="{{ route('student-courses.show', $course1->course) }}"
                                                            class="btn form-control btn-sm  btn-success mb-2">
                                                            View
                                                        </a>
                                                    @else
                                                        <a href="{{ route('student-unenrolled-courses.show', $course1->course) }}"
                                                            class="btn form-control btn-sm  btn-success mb-2">
                                                            View
                                                        </a>
                                                    @endif
                                                ...
                                                <div class="col-6">
                                                    <!-- Unenroll button -->
                                                    <form
                                                        action="{{ route('students.unenrollSelf', ['student' => Auth::user(), 'course' => $course1->course]) }}"
                                                        method="post">
                                                        @csrf
                                                        {{-- <div class="col-12"> --}}
                                                        <button type="submit"
